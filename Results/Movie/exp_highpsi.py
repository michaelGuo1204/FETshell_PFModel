...
def initialize(data):
    ...
    fig_main.suptitle(f"Simulation of FUS-ERG-DNA condensates under higher $\psi_C$")
...
def animate_frame(i,data_sequence):
    ...
    return fig_main
para_hpsi = para.copy()
para_hpsi['Critical'] = 0.95
eta,phi,chi,d_init,p_init = init_dnadiff(para_hpsi,radius=13,dna_level_0=1)
init_order = {"eta":torch.Tensor(eta),"phi":torch.Tensor(phi),"chi":{"0":torch.Tensor(chi["0"])}}
solver = Solver(para_hpsi,init_order,logger=None,target_region="eta-phi",DNA=True,crit=0.15)
solver.initialize()
...
ani.save('animation_exp_highpsi.mp4', writer=ffwriter)
